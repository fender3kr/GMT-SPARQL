...
class InitialConnectedComponents(GraphOperation):
    ...
    def __init__(self, connection):
        GraphOperation.__init__(self, 'Initial Connected Components', connection)
    
    def initialize(self):
        self.connection.urika.update(self.name, InitialConnectedComponents.COMMAND_TO_INITIALIZE_WORKING_GRAPH)
        self.connection.urika.update(self.name, InitialConnectedComponents.COMMAND_TO_INSERT_NODES_TO_WORKING_GRAPH)
    ...
    def process(self):
        ...
        while True:
            self.iteration += 1
            self.connection.urika.update(self.name, InitialConnectedComponents.COMMAND_TO_UPDATE_NODE_TO_MINIMAL_NODE)
            if self.converges():
                break
    ...
    def store(self):
        ...
        results = self.connection.urika.query(self.name, InitialConnectedComponents.COMMAND_TO_RETRIEVE_RESULT, None, None, 'json', True)
        for result in results['results']['bindings']:
            self.writer.writeLine('Node: ' + result['node']['value'] + " [Label: " + result['label']['value'] + "]")

    def finalize(self):
        self.connection.urika.update(self.name, InitialConnectedComponents.COMMAND_TO_FINALIZE_WORKING_GRAPH)
    
    def converges(self):
        count = 0
        converged = False
        results = self.connection.urika.query(self.name, InitialConnectedComponents.COMMAND_TO_CHECK_CONVERGENCE, None, None, 'json', True)
        for result in results['results']['bindings']:
